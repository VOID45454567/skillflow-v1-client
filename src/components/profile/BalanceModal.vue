  ...
  <Modal :model-value="modelValue" @close="$emit('update:modelValue', false)">
    <div class="space-y-5">
      <div class="text-center mb-4">
        ...
        <p class="text-sm text-gray-500">Текущий баланс: {{ currentBalance }} р</p>
      </div>
      ...
      >
        <template #suffix>
          <span class="text-gray-400">р</span>
        </template>
      </BaseInput>

      <div class="p-3 bg-primary/5 rounded-xl border border-primary/10">
        <p class="text-xs text-gray-500 text-center">
          1 р = 1 ₽. Пополнение происходит моментально.
        </p>
      </div>

      <div class="flex gap-3">
        <BaseButton variant="outlined" class="flex-1" @click="close"> Отмена </BaseButton>
        <BaseButton class="flex-1" @click="handleSubmit" :disabled="!amount || Number(amount) <= 0">
          Пополнить
        </BaseButton>
      </div>
    </div>
